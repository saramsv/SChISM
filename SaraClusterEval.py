...
if(len(sys.argv) < 2):
    print("Usage: python3 SaraClusterEval.py labels_file > outputfile")
    sys.exit()
...
num_clusters = 0
for label in sorted(labels.keys()): #for each cluster
    max_count = 0
    num_clusters += 1
    dominant_l  = ""
    #print("Cluster #", label)
    ...
    for gt_l in labels[label]:
        if gt_l not in seen:
            ...
            #print("label: {}, gt_label: {}, count: {}".format(label, gt_l, labels[label].count(gt_l)))
    if dominant_l not in bodypart_accs:
            bodypart_accs[dominant_l] = []
        
    ac = max_count/len(labels[label])*100

    #print("dominant_l: {}, count: {}, total: {}, ac: {}".format(dominant_l, max_count, len(labels[label]), ac))
